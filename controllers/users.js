var {Email, Img} = require('../untils/config.js');
...
var uploadUserImg = async (req, res, next) => {
    console.log(req.file);
    await fs.rename('public/uploads/' + req.file.filename, 'public/uploads/' + req.session.username + '.jpg');
    var result = await UserModel.updatedUserImg(req.session.username, url.resolve(Img.baseUrl, `${req.session.username}.jpg`));
    if(result) {
        res.send({
            msg: '头像修改成功',
            status: 0,
            data: {
                userImg: url.resolve(Img.baseUrl, `${req.session.username}.jpg`)
            }
        });
    }else {
        res.send({
